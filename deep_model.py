    ...
    def __init__(self, name: str, state, action, network_shape: tuple, batch_size: int= 32, trainable: bool = True):
        ...
        with tf.variable_scope(name) as scope:
            ...
            for num, kernel_size, stride in network_shape[:-1]:
                out = tf.layers.conv2d(out, num, kernel_size, stride, activation=tf.nn.relu, trainable = trainable)
                out = tf.layers.max_pooling2d(out, 2, 2)

            # flatten
            fully_connected = tf.reshape(out, [batch_size, -1])
            fully_connected = tf.concat((fully_connected, self.action), axis=1)
            fully_connected = tf.contrib.layers.layer_norm(fully_connected, center=True, scale=True, trainable = trainable)

            # fully connected layer
            for num in network_shape[-1]:
                fully_connected = tf.contrib.layers.fully_connected(fully_connected, num_outputs=num, activation_fn=None, trainable = trainable)
                fully_connected = tf.contrib.layers.layer_norm(fully_connected, center=True, scale=True, trainable = trainable)
                fully_connected = tf.nn.relu(fully_connected)

            self._Q = tf.contrib.layers.fully_connected(fully_connected, 1, activation_fn=None, trainable = trainable)
...
class Mu_Model(object):
    def __init__(self, name: str, state, action_size: int, network_shape: tuple, batch_size: int=32, trainable: bool = True, y_grads=None):
        ...
        with tf.variable_scope(name) as scope:
            ...
            self._a = tf.contrib.layers.fully_connected(fully_connected, action_size, activation_fn=tf.sigmoid, trainable = trainable)
